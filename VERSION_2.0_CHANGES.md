...
- Config vrijednost `placeholder-name` se sada ignoriše

### 5. ✅ Validacija Konfiguracije
**Problem:** Nedostajuće config vrijednosti nisu dobile default vrijednosti
**Rješenje:**
- Dodana metoda `validateConfig()` koja provjerava sve ključne vrijednosti
- Automatski se postavljaju default vrijednosti ako nedostaju
...
- **Java:** 11+ (umjesto 21)
- **Minecraft:** 1.16.5 - 26.2+ (testirana kompatibilnost)
...
Skripт će:
1. Provjeriti Java verziju (trebا 11+)
...
### Kompatibilnost Config-a
Stari `config.yml` će raditi sa verzijom 2.0, ali se preporučuje:

1. Preimenovanje `placeholder-name` vrijednosti (sada se ignoriše)
...
### Performance
- Brže cache lookup-e (debug mode je off po defaultu)
...
- Validacija svih config vrijednosti

## 🐛 Ostali Bugfixes

- Ispravljeno: Null vrijednosti u `placeholder` tokom reload-a
- Ispravljeno: Greške kod offline igrača
- Ispravljeno: Neispravna prikazivanja za male vremenske periode

## 📚 Resursi

- **GitHub:** https://github.com/kusokmedi/donduration
- **Sborka:** `./build.sh` ili `mvn clean package`
